...
package net.jpcode.stfx.manage.controller;
...
import net.jpcode.stfx.util.StringUtils;
...
public class SysMgrController {
    ...
    @RequiresPermissions("sysuser:view")//权限管理;
    @ResponseBody
    public PageList<SysUser> userlistPage(
            @RequestParam(value="username", required=false, defaultValue="")String username,
...
//        mv.setViewName("/manage/sys/rolelist");
    ...
    @RequiresPermissions("sysrole:view")//权限管理;
    @ResponseBody
    public PageList<SysRole> rolelistPage(
            @RequestParam(value="pageno", required=false, defaultValue="1")int pageNo,
            @RequestParam(value="length", required=false, defaultValue="20")int pageSize){
        PageList<SysRole> plist = userService.findRoleList(pageNo, pageSize);
        return plist;
    }
    /**
     * 角色修改_表单查询
